...
#' @export
#'
SSBquantiles <- function(s4files, xlim=c(1931,2032), ylim=c(0,1.4), plot.1989=T, caption="AAA",sub.titles) {
  ...
  for (i in 1:nfiles) {
     ...
     for (yr in ceiling(xlim[1]):floor(xlim[2])) {
        if(length(grep(paste("B.",yr,sep=""),colnames(qdat1)))>=1) {
     ...
     data.columns <- (grep(paste("B.",min.yr,sep=""),colnames(qdat1))):(grep(paste("B.",max.yr,sep=""),colnames(qdat1)) )
     #print(data.columns)
...
#Fig. 12 Plot boxplots for recruitment for a few scenarios
#source("TB10.SSBquantiles.r")
...
#x <- SSBquantiles(s4files=s4files,ylim=c(0,0.42),xlim=c(1980,2021),caption="Spawning biomass by scenario",
