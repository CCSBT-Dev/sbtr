...
plot.SSB.r.quantplots <- function(objects, label="", type=c("obj","grid")) {
   ...
   plot(x=years,y=quants[2,],type="l", ylim=c(0,1.1*max(quants)), col="black",
        lwd=2,xaxt="n",lty=1, yaxs="i", xaxs="i", las=1, ylab="Spawning biomass / unexpl", xlab="Years")
   ...
   Rquants <- apply(depl,MARGIN=2,quantile,c(0.05,0.5,0.95))
   #print(dim(Rquants))
   plot(x=years,y=Rquants[2,],type="l", ylim=c(0,1.1*max(Rquants)), col="black",
        lwd=2,lty=1, yaxs="i", xaxs="i", las=1, ylab="Recruitment / unexpl", xlab="Years")
   lines(x=years,y=Rquants[1,],ylim=c(0,1.2*max(Rquants)), col="black",
         lwd=1,lty=2)
   lines(x=years,y=Rquants[3,],ylim=c(0,1.2*max(Rquants)), col="black",
         lwd=1,lty=2)
   
   invisible(list(SSBquants=quants, Rquants=Rquants))
}
