...
#####NumbersAtAge
###############################
NumbersAtAge <- function(xx, min.age=8,plot.years=c(1931,2013)) {
   x <- xx
...
################################
Indonesian.selectivity <- function(xx, current.yr=2013,do.legend=F,
             sel.years=NULL) {
   if (is.null(sel.years)) {
      year.list=c(1952,seq(1994,current.yr,2))
   }
   else {
      year.list=sel.years
   }
...
compare.plus.groups2 <- function(compare.objects,ylim=c(0,1),
    ...
    for (i in 1:nfiles)
    {
      ...
      NumbersAtAge(xx=xx, min.age=6,plot.years=current.yr)
      if (i==1) { 
         mtext(side=2,outer=F,"2013 numbers (relative)",line=3.5,cex=1.1)  
      ...
      Indonesian.selectivity(xx=xx, current.yr=current.yr, 
                             do.legend=do.legend,
                             sel.years=sel.years)
      if (i==1) {
         axis(2,las=1)
...
#compare.plus.groups2(directory="previous-smoothsqrt",ylim=c(0,1), current.yr=2013)
#dev.off()
