...
###source("LengthsLL1.r")
##########################################################################
LengthsLL1 <- function(labrep.file="sbtmod22_lab.rep", case_label="c1s1l1orig.5_h1m1M1O1C2a1") {
   ...
   obs.data <- x$len.obs[x$len.obs[,1]==1,-c(1,2)]  #extract the lengths where the fishery is =1, and exclude the fishery and year
   pred.data <- x$len.pred[x$len.pred[,1]==1,-c(1,2)]
   years <- x$len.pred[x$len.pred[,1]==1,2]
   nyears <- length(years)
