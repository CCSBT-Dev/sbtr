...
mplist <- c("rh3","rh7","rh8")

# reference/robustness trials

rlist <- c("ref","lowR","highCPUECV")
...
dirnm <- "test_runs" # put the directory name where the .s* files are here
...
for(g in grd) {
  for(t in tun) {
    for(m in mplist) {
      ...
      s4df <- read.table(s4nm,skip=2,header=T)
      s9df <- read.table(s9nm,skip=1,header=T) 
...
pp1 <- ggplot(subset(trodf,stat != trolist[4]),aes(y=val,x=run))+geom_point()+ylim(0,1)+ylab("Probability")
pp1 <- pp1+facet_grid(MP~stat)
pp1

# trend plot

pp2 <- ggplot(subset(trodf,stat == trolist[4]),aes(y=val,x=run,fill=MP))+geom_boxplot(outlier.shape=NA)+ylab(expression(lambda[2021-2035]^{TRO}))+ylim(c(-0.075,0.12))
pp2+facet_grid(. ~ MP)
...
pp3 <- ggplot(subset(tacdf,stat != taclist[2] & stat != taclist[4]),aes(y=val,x=run,fill=MP))+geom_boxplot(outlier.shape=NA)+ylab("Catch stats.")
pp3+facet_grid(stat~MP,scales='free_y')

# AAV

pp4 <- ggplot(subset(tacdf,stat == 'AAV'),aes(y=val,x=run,fill=MP))+geom_boxplot(outlier.shape=NA)+ylab("Percentage")
pp4+facet_grid(.~MP)

# P(C up then down)

pp5 <- ggplot(subset(tacdf,stat == taclist[4]),aes(y=val,x=run,fill=MP))+geom_boxplot(outlier.shape=NA)+ylim(c(0,1))+ylab(taclist[4])
pp5+facet_grid(.~MP)
