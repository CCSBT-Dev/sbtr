...
###source("LengthsLL3.r")
##########################################################################
LengthsLL3 <- function(labrep.file="sbtmod24_lab.rep", case_label="c1s1l1sqrt_h1m1M1O1C2a1") {
   ...
   obs.data <- x$len.obs[x$len.obs[,1]==3,-c(1,2)]  #extract the lengths where the fishery is =3, and exclude the fishery and year
   pred.data <- x$len.pred[x$len.pred[,1]==3,-c(1,2)]
   years <- x$len.pred[x$len.pred[,1]==3,2]
   nyears <- length(years)
   ...
   cohort.color <- rich.colors(nlengths+5)[-c(1:5)]   
   ...
   mtext(side=3,outer=T,line=1.2,"Longline 3 length frequency")
   mtext(side=3,outer=T,line=0.2,paste("(",case_label,")",sep=""),cex=0.6)
}
#pdf("figs\\LengthsLL3.pdf",width=8, height=11.5)
#win.graph(width=7,height=11.5)
#LengthsLL3(labrep.file="sbtmod22_lab.rep",case_label="c1s1l1orig.5_h1m1M1O1C2a1")
#dev.off()
