...
###source("LengthsLL4.r")
##########################################################################
LengthsLL4 <- function(labrep.file="sbtmod24_lab.rep", case_label="c1s1l1sqrt_h1m1M1O1C2a1") {
   ...
   obs.data <- x$len.obs[x$len.obs[,1]==4,-c(1,2)]  #extract the lengths where the fishery is =4, and exclude the fishery and year
   pred.data <- x$len.pred[x$len.pred[,1]==4,-c(1,2)]
   years <- x$len.pred[x$len.pred[,1]==4,2]
   nyears <- length(years)
   ...
   for (yr in 1:nyears) { 
      ...
      x.pos <- par("usr")[1] + 0.1*diff(par("usr")[1:2])   #par("usr") spits out the current coordinates of the plot window
      y.pos <- par("usr")[3] + 0.8*diff(par("usr")[3:4])   #par("usr") spits out the current coordinates of the plot window
      text(x=x.pos,y=y.pos,years[yr],cex=0.9, col=subtle.color)
      par(xpd=T)
   }
   mtext(side=1,outer=T,"Lengths",line=2.3)
   mtext(side=2,outer=T,"Proportions",line=3.2)
   mtext(side=3,outer=T,line=1.2,"Longline 4 length frequency")
