table.C.prob.rel.mult <- function(s3files, relative.to=c(2004), yr.required=c(2014),mult=1, 
              B.option=c("rel.to.single","rel.to.median"),label.names, filenames=T)
{
#TA Branch: 25 Jan 2010 (multiply revised)
#Returns a table containing the scenario and the probability that the C
#in yr.required is greater than the C in year relative.to.
#s3files = vector of .s3 files, including paths, but see FILENAMES below
#relative.to = vector of years
#yr.required = vector of years
#mult = value to multiply relative.to biomass by, e.g. for P(C2022> 0.1*C1980), mult=0.1,relative.to=1980
...
#         = "rel.to.median" is probability greater than median in relative.to year.
#if filename==T then s3files contains names of files to be read in
#if filename==F then s3files contains list of object of read in files using  x <- read.table(s4files[i],skip=2,header=T)
 ...
 prob.gt <- array(data=0,dim=c(nfiles,nyrs),
                dimnames=list(label.names,paste("P(C",yr.required,">",mult,"*C",relative.to,")",sep="")))
 ...
 for (i in 1:nfiles) { 
    ...
    for (j in 1:nyrs) {
       ...
       col.required <- grep(paste("C.",yr.required[j],sep=""),colnames(tmp))
       col.rel.to <- grep(paste("C.",relative.to[j],sep=""),colnames(tmp))
