...
med.rec.base <- function(s4files,B.ylim,xlim=c(1931,2031),leg.names=NULL,legend.x,legend.y)  {
  ...
  results <- matrix(nrow=nfiles,ncol=length(1931:2031))
  colnames(results) <- 1931:2031
  rownames(results) <- leg.names
  ...
  box()
  axis(2,las=1)
  mtext(side=2,"Recruitment (millions)",cex=1.6,line=3)
  ...
  write.csv(file="Tables\\Figure 3 or 25 Rec.csv",results)
}
...
med.SSB.base <- function(s4files,B.ylim,xlim=c(1931,2031),leg.names=NULL,legend.x,legend.y,Bmsy=NULL)  {
  ...
  print("Med SSB20%")
  print(med.B20)
  text(x=2020,y=med.B20+5,"20% of B0",col="gray40")
  if (!is.null(Bmsy)) {
  ...
  axis(1)
  axis(2,las=1)
  mtext(side=1,"Year",cex=1.6,line=2.5)
  mtext(side=2,outer=F,"Spawning stock biomass (thousand mt)",cex=1.6,line=3)
  
  write.csv(file="Tables\\Figure 3 or 25 SSB.csv",results)
}
