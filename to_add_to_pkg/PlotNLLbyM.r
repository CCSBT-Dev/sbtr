...
#split out by different values of M10. 
#Idea and original code: Paige Eveson
#Code modified and rewritten by Trevor A Branch 
#  6 September 2009  tbranch@gmail.com
#Modified 24 June 2014 Trevor A Branch
#To split out by different values of M10 instead of steepness
#######################################################
...
#Plot the likelihoods by M10 values, using Paige's code and idea. 
plot.NLL.by.M <- function(NLLtable,caption) {
   par(mfrow=c(3,4),mai=c(0.1,0.2,0.2,0.2),omi=c(0.6,0.6,0.4,0))
   ...
   for(iii in 1:nplots) {  #the likelihood components
      ...
      plot(x=jitter(NLLtable[,"M10"],amount=0.003),y=NLLtable[,i],xlab="",
                     ylab="",las=1,axes=F,ylim=ylim,pch=19,cex=0.8,col=col.vec+1)
      box()
      axis(2,las=1)
      if (iii>8) {
         axis(1,at=M10.vals)
